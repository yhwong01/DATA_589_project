...
**Reshape `elevation_im`,`forest_im`,`pop_im` to modelling with three covariates.**
...
The p-value \< 2.2e-16 strongly rejects the null hypothesis of Complete Spatial Randomness (CSR). So, deer are not evenly distributed — the intensity (λ) varies across BC. This confirms that modeling intensity as a function of covariates (like elevation, forest, population) is justified.
...
**Now we have `deer_clean`, `forest_clean`, `elev_clean`, `pop_clean`. We could model with these covariates.**

```{r}
library(spatstat)
library(splines)
gam_model_poly <- ppm(deer_clean ~ bs(forest_clean, degree = 3) +
                                   bs(elev_clean, degree = 3) +
                                   bs(pop_clean, degree = 3),
                       covariates = list(
                           forest_clean = forest_clean,
                           elev_clean = elev_clean,
                           pop_clean = pop_clean))
summary(gam_model_poly)


```

```{r}
plot(predict(gam_model_poly), main = "Predicted Deer Intensity")
plot(win_bc, add = TRUE)
```

```{r}
gam_model_poly_2 <- ppm(deer_ppp ~ bs(forest_im, degree = 3) +
                                   bs(elevation_im, degree = 3) +
                                   bs(pop_im, degree = 3),
                       covariates = list(
                           forest_im = forest_im,
                           elevation_im = elevation_im,
                           pop_im = pop_im))
summary(gam_model_poly_2)
```

```{r}
plot(predict(gam_model_poly_2), main = "Predicted Deer Intensity")

```

```{r}
pop_clean_0 <- pop_clean
pop_clean_0$v[is.na(pop_clean_0$v)] <- 0
gam_model_pop0 <- ppm(deer_clean ~ bs(forest_clean, degree = 3) +
                                      bs(elev_clean, degree = 3) +
                                      bs(pop_clean_0, degree = 3),
                      covariates = list(
                          forest_clean = forest_clean,
                          elev_clean = elev_clean,
                          pop_clean_0 = pop_clean_0))
summary(gam_model_pop0)
```
